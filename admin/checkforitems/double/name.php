...
<h3 style="margin-top: 30px;">Дубликаты названий элементов в информационных блоках каталога:</h3>
...
if (!empty($iBlocks_Elements_Name)) {
    $iBlocks_Elements_Repeat = array_count_values($iBlocks_Elements_Name);
    foreach ($iBlocks_Elements_Repeat as $iBlocks_Elements_Value => $iBlocks_Elements_Count) {
        if ($iBlocks_Elements_Count > 1) {
            $iBlocks_Elements_Repeat_Ids = array_keys($iBlocks_Elements_Name, $iBlocks_Elements_Value);
            ?>

            <div style="margin-bottom: 10px;">
                <div>
                    <div>
                        Найдены дубликаты названий: <strong><?= $iBlocks_Elements_Value ?></strong>
                    </div>
                    <div>
                        Количество элементов с данным названием: <strong><?= $iBlocks_Elements_Count ?></strong>
                    </div>
                    <div>
                        Ссылки на редактирование элементов с данным названием:<br/>
                        <?
