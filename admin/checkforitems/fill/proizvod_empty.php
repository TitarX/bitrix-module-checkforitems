...
<h3 style="margin-top: 20px;">Элементы каталога с незаполненным свойством "Производитель":</h3>
...
while ($element_Object = $element_Result->GetNextElement()) {
    ...
    if ($element_Property_Array = $element_Property_Result->GetNext()) {
        if (empty($element_Property_Array['VALUE'])) {
...
if (!empty($iBlocks_Elements_Name)) {
    foreach ($iBlocks_Elements_Name as $iBlocks_Elements_Name_Id => $iBlocks_Elements_Name_Name) {
        ...
        <div style="margin-bottom: 10px;">
            <div>
                <div>
                    Свойство "Производитель" не заполнено в элементе: <strong><?= $iBlocks_Elements_Name_Name ?></strong>
                </div>
                <div>
...
if ($iBlocks_Elements_Repeat_Count === 0): ?>
    <div style="margin-bottom: 10px;">
        Все элементы каталога имеют заполненное свойство "Производитель".
    </div>
<?
endif; ?>
